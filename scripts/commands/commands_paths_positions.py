...
shell_commands = {
    "proxboh": "c:\\temp\\NSSProxySetup.exe -device_type=boh -port=9180 -offline",
    "proxterm": "c:\\temp\\NSSProxySetup.exe -device_type=term -host=ALOHABOH -port=9180 -fallback_ip=",
    "vcr": "C:\\temp\\vcr86.exe /quiet /norestart",
    "checkpanda": """sc query "PandaAetherAgent" | findstr /R /C:"STATE.*RUNNING" && echo RUNNING || sc query "PandaAetherAgent" | findstr /R /C:"STATE.*STOPPED" && echo STOPPED || echo NOT INSTALLED""",
    "rsp": """for /f "tokens=2" %a in ('tasklist /svc ^| findstr PandaAetherAgent') do taskkill /PID %a /F && net start PandaAetherAgent""",
